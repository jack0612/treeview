            ...
            <div #element [attr.id]="_id" [attr.tabindex]="tabindex" [ngStyle]="_style" [class]="_styleClass"
                ...
                <ng-template #buildInContent>
                    <div #content class="p-scroller-content" [ngClass]="{'p-scroller-loading': d_loading}" [ngStyle]="contentStyle">
                        <ng-container *ngFor="let item of loadedItems; let index = index;">
...
export class Scroller implements OnInit, AfterContentInit, AfterViewChecked, OnDestroy {
    ...
    set options(val: ScrollerOptions) {
        ...
        if (val && typeof val === 'object') {
            Object.entries(val).forEach(([k, v]) => (<any>this)[`_${k}`] !== v && ((<any>this)[`_${k}`] = v));
        }
    }
    ...
    scrollToIndex(index: number, behavior: ScrollBehavior = 'auto') {
        ...
        const calculateFirst = (_index = 0, _numT:any) => (_index <= _numT ? 0 : _index);
        const calculateCoord = (_first:any, _size:any, _cpos:any) => (_first * _size) + _cpos;
        const scrollTo = (left = 0, top = 0) => this.scrollTo({ left, top, behavior });

        if (this.both) {
            const newFirst = { rows: calculateFirst((<any>index)[0], numToleratedItems[0]), cols: calculateFirst((<any>index)[1], numToleratedItems[1]) };
    ...
    getRenderedRange() {
        const calculateFirstInViewport = (_pos:any, _size:any) => Math.floor(_pos / (_size || _pos));
    ...
    calculateNumItems() {
        ...
        const calculateNumItemsInViewport = (_contentSize:any, _itemSize:any) => Math.ceil(_contentSize / (_itemSize || _contentSize));
        const calculateNumToleratedItems = (_numItems:any) => Math.ceil(_numItems / 2);
    ...
    calculateOptions() {
        ...
        const calculateLast = (_first:any, _num:any, _numT:any, _isCols = false) => this.getLast(_first + _num + ((_first < _numT ? 2 : 3) * _numT), _isCols);
    ...
    setSize() {
        if (this.elementViewChild?.nativeElement) {
            ...
            const setProp = (_name:any, _value:any) => this.elementViewChild.nativeElement.style[_name] = _value;
    ...
    setSpacerSize() {
        if (this._items) {
            const contentPos = this.getContentPosition();
            const setProp = (_name:any, _value:any, _size:any, _cpos = 0) => this.spacerStyle = { ...this.spacerStyle, ...{ [`${_name}`]: (((_value || []).length * _size) + _cpos) + 'px' } };
    ...
    setContentPosition(pos:any) {
        if (this.contentEl) {
            ...
            const calculateTranslateVal = (_first:any, _size:any) => (_first * _size);
    ...
    onScrollPositionChange(event:any) {
        ...
        const contentPos = this.getContentPosition();
        const calculateScrollPos = (_pos:any, _cpos:any) => _pos ? (_pos > _cpos ? _pos - _cpos : _pos) : 0;
        const calculateCurrentIndex = (_pos:any, _size:any) => Math.floor(_pos / (_size || _pos));
        const calculateTriggerIndex = (_currentIndex:any, _first:any, _last:any, _num:any, _numT:any, _isScrollDownOrRight:any) => {
            return (_currentIndex <= _numT ? _numT : (_isScrollDownOrRight ? (_last - _num - _numT) : (_first + _numT - 1)))
        };
        const calculateFirst = (_currentIndex:any, _triggerIndex:any, _first:any, _last:any, _num:any, _numT:any, _isScrollDownOrRight:any) => {
        ...
        const calculateLast = (_currentIndex:any, _first:any, _last:any, _num:any, _numT:any, _isCols = false) => {
    ...
    handleEvents(name:any, params:any) {
        return this.options && (<any>this.options)[name] ? (<any>this.options)[name](params) : (<any>this)[name].emit(params);
    }
    ...
    getContentOptions() {
        return {
            ...
            getItemOptions: (index:any) => this.getOptions(index),
            loading: this.d_loading,
            getLoaderOptions: (index:any, options?:any) => this.getLoaderOptions(index, options),
    ...
    getLoaderOptions(index:any, extOptions:any) {
